// Minimal SHA-256 (sync) for mock-auth hashing (no native deps).
// Not intended for high-security crypto; use backend auth in production.
...
function toUtf8Bytes(input: string): Uint8Array {
  // TextEncoder is available on modern RN/Expo; fall back if needed.
...
export function sha256Hex(input: string): string {
  ...
  // Append 64-bit length big-endian (we only support up to 2^53-1 bits in JS anyway)
